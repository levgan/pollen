...
@section('content')
    ...
    @can('poll_create')
    <p>
        <a href="{{ route('admin.polls.create') }}" class="btn btn-success">@lang('global.app_add_new')</a>
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.polls.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('global.app_all')</a></li> |
            <li><a href="{{ route('admin.polls.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('global.app_trash')</a></li>
        </ul>
    </p>
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($polls) > 0 ? 'datatable' : '' }} @can('poll_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        @can('poll_delete')
...
@section('javascript') 
    <script>
        @can('poll_delete')
            @if ( request('show_deleted') != 1 ) window.route_mass_crud_entries_destroy = '{{ route('admin.polls.mass_destroy') }}'; @endif
        @endcan
